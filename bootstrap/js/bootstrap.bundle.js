...
(function (global, factory) {
    ...
    const EventHandler = {
        ...
        trigger(element, event, args) {
            ...
            } else evt = new CustomEvent(event, {bubbles, cancelable: true}); // merge custom information in our event
    ...
    const Default$8 = {toggle: true, parent: ''};
    ...
    function arrow(_ref) {
        ...
        let paddingObject = toPaddingObject(options.padding, state),
            ...
            centerToReference = endDiff / 2 - startDiff / 2; // Make sure the arrow doesn't overflow the popper if the center point is
        ...
        let min = paddingObject[minProp],
            ...
            offset = within(min, center, max); // Prevents breaking syntax highlighting...
    ...
    function mapToStyles(_ref2) {
        let _Object$assign2,
            ...
            _ref3 = roundOffsets === true ? roundOffsetsByDPR(offsets) : typeof roundOffsets === 'function' ? roundOffsets(offsets) : offsets,
            _ref3$x = _ref3.x,
    ...
    function computeStyles(_ref4) {
        let state = _ref4.state,
            ...
            commonStyles = {placement: getBasePlacement(state.placement), popper: state.elements.popper, popperRect: state.rects.popper, gpuAcceleration: gpuAcceleration};
    ...
    function getViewportRect(element) {
        let win = getWindow(element),
            ...
            x = 0,
            y = 0; // NB: This isn't supported on iOS <= 12. If the keyboard is open, the popper
    ...
    function detectOverflow(state, options) {
        ...
        let overflowOffsets = {
            ...
            offsetData = state.modifiersData.offset; // Offsets can be applied only to the popper element
    ...
    function distanceAndSkiddingToXY(placement, rects, offset) {
        let basePlacement = getBasePlacement(placement),
            ...
            _ref = typeof offset === 'function' ? offset(Object.assign({}, rects, {placement: placement})) : offset,
            skidding = _ref[0],
            distance = _ref[1];
        skidding = skidding || 0;
    ...
    function preventOverflow(_ref) {
        ...
        if (checkMainAxis || checkAltAxis) {
            let mainSide = mainAxis === 'y' ? top : left,
                ...
                maxLen = variation === start ? -popperRect[len] : -referenceRect[len]; // We need to include the arrow in the calculation so the arrow doesn't go
            ...
            let arrowElement = state.elements.arrow,
                ...
                arrowPaddingMax = arrowPaddingObject[altSide]; // If the reference length is smaller than the arrow length, we don't want
    ...
    const createPopper$1 = /*#__PURE__*/popperGenerator({defaultModifiers: defaultModifiers$1}); // eslint-disable-next-line import/no-unused-modules
    ...
    const createPopper = /*#__PURE__*/popperGenerator({defaultModifiers: defaultModifiers}); // eslint-disable-next-line import/no-unused-modules
    ...
    class Dropdown extends BaseComponent {
        ...
        show() {
            ...
            if (showEvent.defaultPrevented) return; // Totally disable Popper for Dropdowns in Navbar
        ...
        _getPopperConfig() {
            ...
            if (this._config.display === 'static') defaultBsPopperConfig.modifiers = [{name: 'applyStyles', enabled: false}];
    ...
    class Tooltip extends BaseComponent {
        ...
        _setListeners() {
            ...
            if (this._config.selector) this._config = { ...this._config, trigger: 'manual', selector: ''};
            else this._fixTitle();
        }
        _fixTitle() {
        ...
        _getDelegateConfig() {
            ...
            if (this._config) {
                for (const key in this._config) {
                    if (this.constructor.Default[key] !== this._config[key]) config[key] = this._config[key];
                }
            }
            return config;
        }
